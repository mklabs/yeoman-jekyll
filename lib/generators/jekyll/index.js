...
Generator.prototype.updateLayout = function() {
  var layout = path.resolve(this.location, '_layouts/default.html');
...
Generator.prototype.theme = function() {
  ...
  if(this.options.theme === 'tactile') {
    return this.handleTheme(path.join(this.sourceRoot(), 'tactile'), this.location, layout);
  }
  ...
  this.remote(parts[0], parts[1], function(err, remote) {
    if(err) return cb(err);
    self.handleTheme(remote.cachePath, self.location, layout);
    cb();
  });
};
