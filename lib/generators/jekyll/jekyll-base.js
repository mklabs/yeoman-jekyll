...
JekyllBase.prototype.gruntfile = function(config) {
  ...
  this.template('gruntfile.js', 'JekyllBase.js', {
    config: config
  });
};
...
JekyllBase.prototype.handleTheme = function(basedir, outdir, template) {
  var self = this;
  ...
  styles.forEach(function(s) {
    self.copy(path.join(basedir, s), path.join(outdir, s));
  });
  ...
  readmes.concat(htmls).forEach(function(f) {
    ...
    self.copy(path.join(basedir, f), path.join(outdir, out));
  });
};
